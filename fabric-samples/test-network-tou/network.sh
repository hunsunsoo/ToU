...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    infoln "Creating OrgProduct Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgProduct.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
    ...
    infoln "Creating OrgProcess Identities"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-orgProcess.yaml --output="organizations"
    res=$?
    { set +x; } 2>/dev/null
    if [ $res -ne 0 ]; then
      fatalln "Failed to generate certificates..."
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    while :
    do
      if [ ! -f "organizations/fabric-ca/orgProduct/tls-cert.pem" ]; then
        sleep 1
      else
        break
      fi
  ...
  infoln "Generating CCP files for OrgProduct and OrgProcess"
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    ${CONTAINER_CLI} volume rm docker_orderer.tou.com docker_peer0.orgProduct.tou.com docker_peer0.orgProcess.tou.com
    ...
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgProduct/msp organizations/fabric-ca/orgProduct/tls-cert.pem organizations/fabric-ca/orgProduct/ca-cert.pem organizations/fabric-ca/orgProduct/IssuerPublicKey organizations/fabric-ca/orgProduct/IssuerRevocationPublicKey organizations/fabric-ca/orgProduct/fabric-ca-server.db'
    ${CONTAINER_CLI} run --rm -v "$(pwd):/data" busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/orgProcess/msp organizations/fabric-ca/orgProcess/tls-cert.pem organizations/fabric-ca/orgProcess/ca-cert.pem organizations/fabric-ca/orgProcess/IssuerPublicKey organizations/fabric-ca/orgProcess/IssuerRevocationPublicKey organizations/fabric-ca/orgProcess/fabric-ca-server.db'
