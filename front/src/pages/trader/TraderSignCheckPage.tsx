    ...
    <StyledContainer start={backgroundChange}>
      ...
      <StyledMainPaddingContainer>
        ...
        <AnimatedText start={animateActivate}>
          ...
          <FlipContainer>
            {flipStage === 1 && (
              <TextComponent key="text1" start={flipStage === 1}>
                보낼까요?
              </TextComponent>
            )}
            {flipStage === 2 && (
              <TextComponent key="text2" start={flipStage === 2}>
                보내는 중
              </TextComponent>
            )}
            {flipStage === 3 && (
              <TextComponent key="text3" start={flipStage === 3}>
                보냈어요 !
              </TextComponent>
            )}
          </FlipContainer>
...
const StyledContainer = styled.div<StyledContainerProps>`
  ...
  background-image: ${(props) =>
    props.start
      ? "linear-gradient(0deg, #FFF 6.72%, #CEEDFF 37.49%, #FFF 85.23%);"
      : "none"};
`;
...
const AnimatedText = styled.div<AnimatedTextProps>`
  ...
  animation: ${(props) =>
    props.start
      ? css`
          ${shrinkMoveFont} 0.8s ease-in-out forwards
        `
      : "none"};
`;
...
const AnimatedHeader = styled.div<RemoveBarProps>`
  animation: ${(props) =>
    props.start
      ? css`
          ${fadeOutUp} 1s ease-in-out forwards
        `
      : "none"};
  ${(props) =>
    props.end &&
